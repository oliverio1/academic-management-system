<form method="GET" action="{{ route('coordination.prefect-reports.index') }}" class="coordination-report-filters">
    <div class="row">
    ...
    <a href="{{ route('coordination.prefect-reports.index') }}" class="btn btn-outline-secondary">Limpiar</a>
</form>
...
<div class="table-responsive coordination-report-table-wrap">
    <table data-datatable="true" class="table table-hover mb-0 reports-table">
        ...
        <tbody>
                ...
                <tr class="{{ $rowClass }}">
                    ...
                    <td>
                        <div class="report-actions">
                            @if($report->status === 'open')
                                <form method="POST" action="{{ route('coordination.prefect-reports.update-status', $report) }}">
                                    @csrf
                                    @method('PATCH')
            ...
            @empty
                <tr><td colspan="8" class="text-center text-muted py-4">No hay reportes registrados.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
