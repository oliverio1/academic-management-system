<div class="card mb-3">
    ...
    <div class="card-body">
        <form method="GET" action="{{ route('coordination.student-incident-reports.index') }}">
            <div class="row">
...
<div class="card">
    <div class="card-body table-responsive p-3">
        <table data-datatable="true" class="table table-hover mb-0">
            <thead>
                <tr>
                    <th>Fecha</th><th>Alumno</th><th>Grupo</th><th>Destinatario</th><th>Categoría</th><th>Asunto</th><th>Descripción</th><th>Estatus</th><th>Acciones</th>
                </tr>
            </thead>
            <tbody>
                @forelse($reports as $report)
                    <tr>
                        ...
                        <td>
                            @if($report->status === 'open')<span class="badge badge-danger">Pendiente</span>
                            @elseif($report->status === 'reviewed')<span class="badge badge-warning">Revisado</span>
                            @else<span class="badge badge-success">Resuelto</span>@endif
                        </td>
                        <td>
                            @if($report->status !== 'resolved')
                                <form method="POST" action="{{ route('coordination.student-incident-reports.update-status', $report) }}" class="mb-1">
                                    @csrf @method('PATCH')
                                    <input type="hidden" name="status" value="reviewed">
                                    <button class="btn btn-sm btn-outline-warning">Marcar revisado</button>
                                </form>
